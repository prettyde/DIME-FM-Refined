...
def create_dataloader(dataset, batch_size, shuffle=True, num_workers=2, pin_memory=True):
...
def get_dataloader(dataset, val_split=0.0, batch_size_per_gpu=64, workers=2, pin_memory=True):
    import platform
...
def load_custom_zeroshot_model(config):
    ...
    new_state_dict = {}
    for k, v in state_dict.items():
        new_k = k.replace('lang_encoder', 'text').replace('text_encoder', 'text').replace('lang_projection', 'text_projection').replace('image_encoder', 'visual').replace('image_projection', 'vision_projection')
