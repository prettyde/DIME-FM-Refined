...
class MemoryBankModule(torch.nn.Module):
    ...
    def _init_memory_bank(self, dim: int):
        ...
        self.bank = torch.randn(dim, self.size).cuda().half()
        self.bank = self.bank / (self.bank.norm(dim=0, keepdim=True)+1e-10)
    ...
    def _dequeue_and_enqueue(self, batch: torch.Tensor):
        ...
        while ptr + batch_size >= self.size:
            self.bank[:, ptr:] = batch[:self.size - ptr].T.detach()
            batch = batch[self.size - ptr:]
            self.bank_ptr[0] = 0
